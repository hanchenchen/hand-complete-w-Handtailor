...
mano2bighand_skeidx = [
    0, 13, 1, 4, 10, 7, 14, 15, 16, 2, 3, 17, 5, 6, 18, 11, 12, 19, 8, 9, 20
]
...
class MANO_SMPL(nn.Module):
    ...
    def forward(self, beta, theta, quat, get_skin=False, use_pca=True):
        ...
        T = torch.matmul(W, A.view(num_batch, 16,
                                   16)).view(num_batch, -1, 4, 4)
        ...
        if get_skin:
            return verts, joints, torch.cat([self.quat2mat(quat).view(-1, 1, 3, 3), Rs], dim=1)
        else:
            return joints
    ...
    def batch_global_rigid_transformation(self,
        ...
        def make_A(R, t):
            ...
            t_homo = torch.cat([t, Variable(torch.ones(N, 1, 1)).to(Rs.device)],
                               dim=1)
    ...
    def get_mano_vertices(self, quat, pose, shape, cam):
        """
