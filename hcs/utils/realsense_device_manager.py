...
def post_process_depth_frame(depth_frame, decimation_magnitude=1.0, spatial_magnitude=2.0, spatial_smooth_alpha=0.5,
    ...
    temporal_smooth_alpha: double
                           The alpha value for temporal filter baed smoothening
    ...
    # Avaiable filters and control options for the filters
    decimation_filter = rs.decimation_filter()
    spatial_filter = rs.spatial_filter()
    temporal_filter = rs.temporal_filter()

    filter_magnitude = rs.optiopn.filter_magnitude
    filter_smooth_alpha = rs.option.filter_smooth_alpha
    filter_smooth_delta = rs.option.filter_smooth_delta

    # Apply the control paramters for the filter
...
class DeviceManager(object):
    ...
    def load_settings_json(self, path_to_settings_file):
        ...
        for (device_serial, device) in self.enable_devices.items():
    ...
    def poll_frames(self):
        """
        Poll for frames from the enabled Intel Realsense devices. This will retur at least one frame from each device.
        If temporal post processing is enabled, the depth stream is averaged over a centain amount of frames
