...
def torch_train(train_loader, model, update_fn, _config, callbacks=[]):
    ...
    for epoch in range(epoch_len):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            # print("data_key:{}",format(data.keys()))

            # print("temporal_pose2d_shape:{}".format(data["temporal_pose2d"].shape))
            # print("pose3d_shape:{}".format(data["pose3d"].shape))
            # print("valid_pose_shape:{}".format(data["valid_pose"].shape))
