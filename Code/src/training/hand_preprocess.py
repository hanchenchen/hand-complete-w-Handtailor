...
from databases.joint_sets import Common14Joints, CocoExJoints, HandJoints
...
def preprocess_2d(data, fx, cx, fy, cy, joint_set, root_name):
    ...
    # print("Num_Joints:{}".format(joint_set.NUM_JOINTS))
    ...
    # print("normalized_2d:{}".format(data))
    return data
...
class BaseNormalizer(object):
    ...
    def __call__(self, sample):
        ...
        # print("nomalized_sample_2d:{}".format(sample[self.field_name]))
        return sample
...
class MeanNormalize2D(BaseNormalizer):
    ...
    def __init__(self, dataset):
        ...
        # print("nomarlized_2d:{}".format(data))
...
def get_postprocessor(config, test_set, normalizer3d):
    if config['preprocess_3d'] == 'SplitToRelativeAbsAndMeanNormalize3D':
        def f(x, seq):
            scale = 1 if isinstance(test_set.pose3d_jointset, HandJoints) else 1000
