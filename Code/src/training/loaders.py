...
from databases.hand_datasets import Hand3dTrainDataset
...
class ChunkedGenerator:
    ...
    def __init__(self, dataset, batch_size, pad, augment, shuffle=True):
        ...
        """
        assert isinstance(dataset, (Mpi3dTrainDataset, PersonStackedMucoTempDataset, ConcatPoseDataset, Hand3dTrainDataset)), "Only works with Mpi datasets"
        self.dataset = dataset
        ...
        N = len(dataset.index)
        # print("len(dataset.index) = {}".format(N))
        ...
        # print("frame_start_len = {}, frame_end_len = {}".format(len(frame_start), len(frame_end)))

        # print("frame_start = {}, frame_end = {}".format(frame_start, frame_end))
    ...
    def __iter__(self):
        ...
        class LoadingDataset:
            ...
            def __getitem__(iself, ind):
                ...
                # print("chunk_inds_shape:{}, chunk.shape:{}".format(chunk_inds.shape, chunk['pose2d'].shape[1:]))
