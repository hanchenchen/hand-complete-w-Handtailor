...
from databases.joint_sets import CocoExJoints, OpenPoseJoints, MuPoTSJoints, HandJoints

from databases import huawei_hand
...
class AugmentMixin:
    def augment(self, scale_by_dist, scales=None):
        ...
        root_ind = HandJoints().index_of('hip')
...
class TemporalAugmentMixin(AugmentMixin):
    ...
    def augment(self, scale_by_dist, scales=None):
        ...
        if scales is None:
            ...
            #TODO: modify the root from hip to 1
...
class FlippableDataset(PoseDataset):
    ...
    def get_samples(self, ind, flip):
        ...
        # print("unfilp_pose_2d:{}".format(sample["pose2d"]))
        # TODO：delete flip
        ...
        # print("flip_pose_2d:{}".format(sample["pose2d"]))
        return sample
...
class Hand3dTestDataset(FlippableDataset):
    def __init__(self, pose2d_type, pose3d_scaling, eval_frames_only=False):
        ...
        self.pose2d_jointset = HandJoints()
        self.pose3d_jointset = HandJoints()
        calibs = huawei_hand.get_calibration_matrices()
        ...
        # width = []
        index = []
        #range ceil is 21 for sub and 25 for seq
        for sub in range(15, 16):
            for seq in range(1, 2):
                gt = huawei_hand.train_ground_truth(sub, seq)
                if gt is None:
                    continue
                print("load data: {}".format((sub, seq)))
                for cam in range(4):
                    ...
                    tmp_pose3d = []
                    for i in inds:
                        tmp_pose3d.append(gt[pose3d_key][cam][i])

                    poses3d.append(tmp_pose3d)


                    #TODO: load hrnet predict 2d pose
                    ...
                    tmp_valid2d = []
                    for i in inds:
                        tmp_pose2d.append(gt["annot2"][cam][i])
                        tmp_valid2d.append(True)
                    poses2d.append(tmp_pose2d)
                    valid_2d_pred.append(tmp_valid2d)
                    # print(len(poses2d)

                    assert len(poses3d[-1]) == len(poses2d[-1]
                                                   ), "Gt and predicted frames are not aligned, seq:" + str(seq)

                    index.extend([(sub, seq, cam, i) for i in inds])

                    calibration_mx = calibs[(sub, 0, cam)]
        ...
        self.index = np.rec.array(index, dtype=[('seq', 'int32'), ('sub', 'int32'),
...
class Hand3dTrainDataset(FlippableDataset, TemporalAugmentMixin):
    def __init__(self, pose2d_type, pose3d_scaling, stride=1):
        assert pose2d_type == 'hrnet', "Only hrnet 2d is implemented"
        assert pose3d_scaling in ['normal', 'univ'], \
            "Unexpected pose3d scaling type: " + str(pose3d_scaling)
        self.transform = None
        ...
        # print("self.poese2d:{}".format(self.poses2d))
        # print("self.poese3d:{}".format(self.poses3d))
        # print("self.cx:{}".format(self.cx))

        #TODO: this step is right, self.pose2d and self.pose3d are correct
